...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 20 14:52:06 2019
...
while date1 < today:
   ...
   ##print(date1)
   cur.execute("SELECT a.subject_gen_subject, cast(avg(a.lesson_no) as FLOAT), max(a.week_num) as week from (SELECT lo.fle_student_dw_id,lo.subject_gen_subject, cast(count(distinct lo.fle_lo_dw_id) as FLOAT) as lesson_no, date_part('week',max(lo.full_date))-2 as week_num from alefdw.learning_experience_view lo where lo.full_date >= 2019-01-13 and lo.full_date < %s and lo.fle_lesson_type='SA' and lo.fle_tenant_dw_id=2 and lo.fle_attempt=1 and lo.school_name!='Alef School' group by lo.fle_student_dw_id,lo.subject_gen_subject) a group by a.subject_gen_subject",(date2))
